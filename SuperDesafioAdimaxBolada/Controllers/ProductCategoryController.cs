...
public class ProductCategoryController : ControllerBase
{
    ...
    [HttpGet]
    public IEnumerable<ReadProductCategoryDTO> GetRelationship([FromQuery] int? productId = null)
    {
        if (productId == null)
        {
            return _mapper.Map<List<ReadProductCategoryDTO>>(_context.ProductsCategories.ToList());
        }
        return _mapper.Map<List<ReadProductCategoryDTO>>(_context.ProductsCategories.FromSqlRaw($"SELECT * FROM ProductsCategories where ProductsCategories.ProductId = {productId}").ToList());
    }
